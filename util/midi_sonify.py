...
def mix_audio(*audios: np.ndarray) -> np.ndarray:
    """Sum multiple audio arrays and normalize to [-1, 1].

    Arrays are zero-padded to the length of the longest input.
    Works with audio from any source (MIDI, ABC, WAV, etc.).
    """
    if not audios:
    ...
    for a in audios:
        mixed[:len(a)] += a.astype(np.float64)
    return _normalize(mixed)


def merge_midi(*pms: pretty_midi.PrettyMIDI) -> pretty_midi.PrettyMIDI:
    """Combine multiple PrettyMIDI objects into one.

    Instruments from all inputs are collected into a single PrettyMIDI,
    using the tempo/time-signature from the first input. Useful for
    combining parts from different scores or files before synthesis.
    """
    if not pms:
        return pretty_midi.PrettyMIDI()
    merged = _copy_pm_skeleton(pms[0])
    for pm in pms:
        for inst in pm.instruments:
            merged.instruments.append(copy.deepcopy(inst))
    return merged
